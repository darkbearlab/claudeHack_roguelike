...
**Play:** https://darkbearlab.github.io/claudeHack_roguelike/classic/

One of two games in this repository - see the [repository README](../README.md) for the
other one and for how they share an engine.
...
Then open http://localhost:8778/classic/. `tools/devserver.py` sends `Cache-Control:
...
(base64 PNG) into `docs/img/` at the repository root, which is how the
...
```bash
node tools/systest.mjs                 # 22 system checks; the real suite
...
classic/
  ...
  tools/              this game's test harnesses
  docs/               design notes, the Chinese guide
../engine/            seeded RNG, helpers, field of view, pathfinding (shared)
../assets/            57 generated sprites (PNG) with their recipes (shared)
```

`engine/` holds only the modules with no game knowledge at all. Everything else
is claudeHack's, and claudeSouls keeps its own copy - see the
[repository README](../README.md) for why the boundary is drawn there.
